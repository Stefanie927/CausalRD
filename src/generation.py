...
from util.datasets import TestDataset_diffusion
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output_dir/causalRD_gen/',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_dir/causalRD_gen/',
...
def main(args):
    ...
    dataset_test = TestDataset_diffusion(test_df, test_encoder_feature_pkl, test_img_dir, is_train=False, args=args)
    ...
    for image_diff, encoder_feat, image_id, targets, image_name in data_loader_test:
        ...
        cond = model.causal_model.get_single_features(encoder_feat, targets)
        cond = cond.view(cond.shape[0], -1)  # flatten
        # based on reverseDDIM generation
        xT = encode_stochastic(model, image_diff, conf, cond, T=250)
        sample = render(model, xT, conf, cond, T=100)
        for ix in range(sample.shape[0]):
            save_image(sample[ix], rf"{image_name[ix]}.png")
