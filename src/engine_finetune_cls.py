...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    print(model.DisentanglementEncoder.A)
    for data_iter_step, (image_feat, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            outputs, output_age, output_sex, mask_recon_loss, pred_o_loss, discriminator_loss, prior_kl, feat_kl, mask_kl, label_mse = model(image_feat, targets)
            # loss = criterion(outputs, targets)
            loss = pred_o_loss*20 + discriminator_loss + prior_kl + mask_recon_loss + feat_kl + mask_kl + label_mse
        ...
        metric_logger.update(mask_recon_loss=mask_recon_loss.item())
        metric_logger.update(pred_o_loss=pred_o_loss.item())
        metric_logger.update(discriminator_loss=discriminator_loss.item())
        metric_logger.update(prior_kl=prior_kl.item())
        metric_logger.update(feat_kl=feat_kl.item())
        metric_logger.update(mask_kl=mask_kl.item())
        metric_logger.update(label_mse=label_mse.item())
        min_lr = 10.
        max_lr = 0.
...
def ODIR_Metrics(gt_data, pr_data):
    ...
    precisions = metrics.precision_score(gt, pr>th)
    recalls = metrics.recall_score(gt, pr>th)

    return kappa, auc, precisions, recalls
...
def evaluate(model, dataloader, device, criterion):
    ...
    val_loss = 0
    with torch.no_grad():
        for image_feat, labels in dataloader:
            image_feat = image_feat.to(device)
            labels = labels.to(device)

            outputs, output_age, output_sex, mask_recon_loss, pred_o_loss, discriminator_loss, prior_kl, feat_kl, mask_kl, label_mse = model(image_feat, labels)
            output = outputs
...
def prediction(model, dataloader, device):
    ...
    all_probs = []
    with torch.no_grad():
        for image_feat, img_id, labels in dataloader:
            image_feat = image_feat.to(device)
            labels = labels.to(device)

            outputs, output_age, output_sex = model.test_forward(image_feat, labels)
            output = outputs
            all_labels.append(labels[:,2:].cpu().numpy())   
    ...
    kappa, auc, precisions, recalls = ODIR_Metrics(all_labels, all_probs)   
...
@torch.no_grad()
def save_features(model, dataloader, device):
    model.eval()
    features_dict = {}
    ...
    with torch.no_grad():
        for image_feat, img_id, labels in dataloader:
            ...
            left_concept_embs, right_concept_embs = model.get_features(image_feat, labels)
            concept_embs = torch.cat([left_concept_embs, right_concept_embs], dim=-1)
            
            if int(img_id.cpu().numpy()[0]) not in features_dict.keys():
                features_dict[int(img_id.cpu().numpy()[0])] = concept_embs.cpu().numpy()
